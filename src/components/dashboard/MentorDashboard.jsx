...
export default function MentorDashboard({ user, suggestions, upcomingSessions, mentorships, loading }) {
  return (
      ...
      <div className="lg:col-span-2 space-y-6 md:space-y-8">
        ...
        <Card gradient hover padding="lg">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button size="sm" variant="outline" icon={<Plus className="w-4 h-4" />} onClick={() => window.location.href = '/find-mentors'}>
              Find More
            </Button>
          </div>
          ...
          {loading ? (
            ...
            <EmptyState icon={Users} title="No suggestions yet" description="Complete your profile to receive personalized mentor recommendations" action={
              <Button variant="orange" size="sm" onClick={() => window.location.href = '/find-mentors'}>Explore Mentors</Button>
            } />
          )}
        </Card>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {upcomingSessions.map((session) => (
                <div key={session.id} className="group flex items-center gap-4 p-4 bg-gradient-to-br from-white to-blue-50/50 rounded-[20px] border border-blue-100/50 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 cursor-pointer">
                  <div className="relative">
